...
DWORD WINAPI ThreadProc(LPVOID lpParam)
{
	...
	ofstream outTXT(string("out")+to_string(count) + ".txt");
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < threadCount; i++)
	{
		...
		SetThreadAffinityMask(handles[i], static_cast<DWORD_PTR>(1) << (i/kernelCount));
	}

	for (int i = 0; i < threadCount; ++i)
	{
		SetThreadPriority(handles[i], atoi(argv[5 + i]));
		cout << i << " " << atoi(argv[5 + i]) << endl;
	}
